...
class l1BridgeMonitorService extends OptimismEnv {
  constructor() {
    ...
    this.endBlock =
      Number(this.startBlock) + Number(this.l1BridgeMonitorLogInterval)
  }
  ...
  async initConnection() {
    ...
    const startBlock = (
      await this.databaseService.getNewestBlockFromL1BridgeTable()
    )[0]['MAX(blockNumber)']
    if (startBlock) {
      ...
      for (const eachL1LPLog of L1LPLog) {
        ...
        if (
          ...
          if (L1LPEvent.name === 'ClientDepositL1') {
            ...
            const depositReceive = L1LPEvent.args.receivedAmount
              .sub(
                L1LPEvent.args.receivedAmount
                  .mul(totalFeeRate)
          ...
          const payload = {
            ...
            crossDomainMessageEstimateFinalizedTime,
            crossDomainMessageFinalizedTime: null,
            timestamp,
            fastDeposit,
          }
          ...
          this.logger.info(
            `Found L1 LP logs found from block ${this.startBlock} to ${endBlock}`
          )
        }
      }
    } else {
    ...
    if (L1StandardBridgeLog.length) {
      for (const eachL1StandardBridgeLog of L1StandardBridgeLog) {
        ...
        if (
          ...
          const crossDomainMessageEstimateFinalizedTime =
            timestamp + Number(this.l1CrossDomainMessageWaitingTime)
  ...
  async startCrossDomainMessageMonitor() {
    ...
    for (const eachCrossDomainMessage of crossDomainMessages) {
      ...
      if (l2Message !== false) {
        ...
        const blockData = await this.L2Provider.getBlockWithTransactions(
          l2BlockNumber
        )
        ...
        if (eachCrossDomainMessage.fastDeposit) {
          const L2LiquidityPoolLog =
            await this.L2LiquidityPoolContract.queryFilter(
              this.L2LiquidityPoolContract.filters.ClientPayL2(),
              Number(l2BlockNumber),
              Number(l2BlockNumber)
            )

          const filteredL2LiquidityPoolLog = L2LiquidityPoolLog.filter(
            (i) => i.transactionHash === l2Hash
          )
          if (filteredL2LiquidityPoolLog.length) {
            this.logger.info('Found successful L2 deposit', {
              status: 'succeeded',
              hash: eachCrossDomainMessage.hash,
              blockNumber: eachCrossDomainMessage.blockNumber,
            })
            ...
            this.logger.info('Found failure L2 deposit', {
              status: 'reverted',
              hash: eachCrossDomainMessage.hash,
              blockNumber: eachCrossDomainMessage.blockNumber,
            })
        ...
        this.logger.info(
          `Found L1 LP cross domain message ${eachCrossDomainMessage}`
        )
      } else {
  ...
  async getL1ToL2TransactionReceipt(l1ToL2msgHash) {
    ...
    const logs = await this.L2ProviderReplica.getLogs(filter)
    const matches = logs.filter((log) => log.topics[1] === l1ToL2msgHash)

    if (matches.length > 0) {
      if (matches.length > 1) {
        this.logger.warn(
          `Found multiple transactions relaying the same message hash ${l1ToL2msgHash}.`
        )
        return false
      }
      return matches[0]
